...
function generatEmailToken(): string {
...
router.post('/login', async(req, res) => {
    ...
    // generate token
    const emailToken = generatEmailToken();
...
router.post('/authenticate', async(req, res) => {
   ...
   if(dbEmailToken.expiration < new Date()){
     return res.status(401).json({ error: "Token exprired"});
   }
   ...
   // here we  validated that the user is the owner of email
